  ...
  for(i = nb_args; i-- ;) {
    ...
    switch(vtop[-i].type.t & VT_BTYPE) {
      ...
      if (float_abi == ARM_HARD_FLOAT) {
        ...
        if (is_float(vtop[-i].type.t)
          ...
          if (start_vfpreg >= 0) {
            pplan = (struct param_plan) {start_vfpreg, end_vfpreg, &vtop[-i],0};
            if (is_hfa)
	 ...
	 * CORE_STRUCT_CLASS or the first of STACK_CLASS. */
        for (j = ncrn; j < 4 && j < ncrn + size / 4; j++)
          *todo|=(1<<j);
        pplan = (struct param_plan) {ncrn, j, &vtop[-i],0};
      ...
      if (ncrn < 4) {
        ...
        pplan = (struct param_plan) {ncrn, ncrn, &vtop[-i],0};
        ncrn++;
        if (is_long)
          pplan.end = ncrn++;
    ...
    pplan = (struct param_plan) {nsaa, nsaa + size, &vtop[-i],0};
