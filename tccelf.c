...
{
    ElfW(Dyn) *dyn;
    dyn = (ElfW(Dyn) *)section_ptr_add(dynamic, sizeof(ElfW(Dyn)));
    dyn->d_tag = dt;
    dyn->d_un.d_val = val;
}
...
static int tcc_add_support(TCCState *s1, const char *filename)
{
    ...
    return tcc_add_binary_file(s1, buf, AFF_TYPE_BIN | AFF_PRINT_ERROR);
}
...
static int ld_add_file(TCCState *s1, const char filename[])
{
    if (filename[0] == '/') {
        if (CONFIG_SYSROOT[0] == '\0'
                && tcc_add_binary_file(s1, filename, AFF_TYPE_BIN) == 0)
            return 0;
                                    ...
                                    const char *filename, int flags, char **paths, int nb_paths)
{
    ...
    for(i = 0; i < nb_paths; i++) {
        ...
        if (tcc_add_binary_file(s, buf, flags | AFF_TYPE_BIN) == 0)
            return 0;
    }
    return -1;
}
