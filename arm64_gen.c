...
void gfunc_call(int nb_args)
{
    ...
    for (i = nb_args; i; i--, vtop--) {
        if (a[i] < 16 && !(a[i] & 1)) {
            ...
            if ((vtop->type.t & VT_BTYPE) == VT_STRUCT) {
                int sz;
                uint32_t j, n = arm64_hfa(&vtop->type, &sz);
                vtop->type.t = VT_PTR;
                gaddrof();
                gv(RC_R30);
                for (j = 0; j < n; j++)
...

    {
        ...
        if (bt == VT_BYTE || bt == VT_SHORT)
            ...
            if (a[0] == 0) {
                ...
                int sz;
                uint32_t j, n = arm64_hfa(return_type, &sz);
                for (j = 0; j < n; j++)
                    o(0x3d000100 |
                      (sz & 16) << 19 | -(sz & 8) << 27 | (sz & 4) << 29 |
                      (a[i] / 2 - 8 + j) |
...
void gfunc_prolog(CType *func_type)
{
    ...
    for (i = 1, sym = func_type->ref->next; sym; i++, sym = sym->next) {
        ...
        if (16 <= a[i] && a[i] < 32 && (sym->type.t & VT_BTYPE) == VT_STRUCT) {
            int sz;
            uint32_t j, k = arm64_hfa(&sym->type, &sz);
            if (sz < 16)
                for (j = 0; j < k; j++) {
...
int gfunc_sret(CType *vt, int variadic, CType *ret,
